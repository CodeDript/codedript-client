...
import styles from './Button3Black1.module.css';
...
const Button: React.FC<ButtonProps> = ({ text = "View more", onClick, className }) => {
    ...
    const uniqueId = useRef(`btn-${Math.random().toString(36).substr(2, 9)}`);
    const gradientId = useRef(`buttonGradient-${uniqueId.current}`);
    const maskId = useRef(`mask-${uniqueId.current}`);
